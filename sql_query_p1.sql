...
rank() over(partition by year(sale_date) order by avg(total_sale) desc) as r1
from retail_sales
...
-- here 1 refers to first column entered in select statment and so on...
...
from retail_sales)
select shift, count(*) no_of_orders from shift_wise_sale
group by Shift;
